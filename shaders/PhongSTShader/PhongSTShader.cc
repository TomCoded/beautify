...
#define ReflectionVectorPlusDirOfLight \
  ...
  //get the reflection std::vector
  Point3Dd reflection(((DirToLight)*-1)+ReflectionVectorPlusDirOfLight);
  //get cosine between reflection std::vector and sample ray
  cosf =
...
Point3Dd PhongSTShader::getColor(Hit &hitPoint)
{
  ...
  std::vector<Light *>::iterator itVisibleLights;

  //Get a std::vector of all Lights in the Scene that are at all visible.
...
std::istream& PhongSTShader::in(std::istream&)
{
}

std::ostream& PhongSTShader::out(std::ostream&)
{
}
